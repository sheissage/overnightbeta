...
def updateRoomInfo(request):
    if request.method == 'POST':
        ...
        roomobj = models.RoomInfo(Destination=destination,roomType=roomType,date=date,airportTransfer=price,ratePerNight=price,discountPercent=discount,hoteltax=price,servicecharge=price)
        roomobj.save()
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        print('username')
        for each in roomarr:
            room = models.RoomInfo(HotelName=name,date='2016-06-01',ownerId=request.user,roomType=each.upper(),Destination=destination,ratePerNight=0,packagePrice=0,discountPercent=0,airTransfer=0,serviceCharges=0,tax=0)
            room.save()

        hotel = models.HotelInfo(HotelID = hotelId,Destination=destination,HotelAddress=address,HotelAmens=amenities,HotelServices=services)
        hotel.save()
...
def createMerchant(request):
    ...
    if request.method =='POST':
        ...
        if repass==password:
            ...
            subject, from_email, to = 'Welcome to Overnight.asia (beta)', 'enquiry@overnight.asia', email
            text_content = 'Thank you for signing up as a partner. '+ fname+"\n You're one of the #Overnight20 partners."
...
def  signupUser(request):
    if request.method=='POST':
        ...
        if repass == passwd:
            ...
            newTraveller.save()
            subject, from_email, to = 'Welcome to Overnight.asia (beta)', 'enquiry@overnight.asia', email
            text_content = 'Hello! ' + fname + "\n Thank you for signing up as a beta member of Overnight.asia."
            html_content = '<p>Hello! ' + fname + '<br> Thank you for signing on <strong>Overnight.asia</strong> .</p>'
            msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
            msg.attach_alternative(html_content, "text/html")
            msg.send()
            return render(request,"login.html")
...
def callPriceRefresh(request):
    ...
    hoteldata = models.HotelInfo.objects.all().filter(HotelName=hotelName).values()
    pricebydate = models.RoomInfo.objects.all().filter(HotelName=hotelName,roomType=roomType).values('ratePerNight')
    roomdata = models.RoomInfo.objects.all().filter(HotelName=hotelName).values()
    print(roomdata)
    return render(request, 'bookingdetails.html', {'hoteldata': hoteldata, 'roomdata': roomdata})
